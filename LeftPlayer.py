...
class LeftPlayer(Player):
    def __init__(self, x, y, game, movement_speed, attacking_speed, attacking_range, defending_range, blocking_time):
        """
        Sous classe de Player permettant de créer un objet LeftPlayer.
        Possède les même paramètres que la classe Player.
        Permet de gérer les mouvements du joueur 1.
        """
    ...
    def jump_left(self):
        ...
        if self.game.menu == False:
            if len(self.current_action) == 0 and self.x >= 4 and self.game.screen[-2][self.x-3] != "#" and self.game.screen[-2][self.x-4] != "#":
    ...
    def jump_right(self):
        ...
        """
        if self.game.menu == False:
            if len(self.current_action) == 0 and self.x <= len(self.game.screen[0])-6 and self.game.screen[-2][self.x+3] != "#" and self.game.screen[-2][self.x+2] != "#":
                for i in range((self.movement_speed-1)*3):
                    self.current_action.append(())
                if self.movement_speed == 1:
    ...
    def moveRight(self):
        ...
        """
        if self.game.menu == False:
            if len(self.current_action) == 0 and self.x <= len(self.game.screen[0])-4 and self.game.screen[self.y+4][self.x+1] != "#":
    ...
    def moveLeft(self):
        ...
        """
        if self.game.menu == False:
            if len(self.current_action) == 0 and self.x >= 2 and self.game.screen[self.y+4][self.x-2] != "#":
                for i in range(self.movement_speed-1):
                    self.current_action.append(())
                self.current_action.append((self.update, False))
    ...
    def print(self, attack, block):
        ...
        self.game.screen[self.y+1][self.x+2] = "\033[0;32m" + "/" + "\033[00m"
        if attack:
            self.game.screen[self.y+1][self.x+2] = "\033[0;32m" + "_" + "\033[00m"
            for i in range(1, self.attacking_range):
                if self.x+2+i > len(self.game.screen[0])-1:
                    break
                if self.game.screen[self.y+1][self.x+2+i] == " ":
                    self.game.screen[self.y+1][self.x+2+i] = "\033[0;32m" + "_" + "\033[00m"
        if block:
            self.game.screen[self.y+1][self.x+2] = "\033[0;32m" + "|" + "\033[00m"
    ...
    def update_attack(self):
        ...
        """
        self.game.leftPlayerAttacking = True
        for i in range(1, self.attacking_range):
            if self.x+2+i > len(self.game.screen[0])-1: # or self.game.screen[self.y+1][self.x+2+i] == "\033[0;31m" + "|" + "\033[00m":
                break
            if self.game.screen[self.y+1][self.x+2+i] == "_" or self.game.screen[self.y+1][self.x+2+i] == "|":
                self.game.rightPlayerAttacked = True
    ...
    def update_block(self):
        ...
        """
        self.game.leftPlayerBlocking = True
